...
classified_injuries %>%
  ...
  kable(
    caption = "Table 1: Breakdown of Injury Costs by Cause and Work Environment.",
...
fall_data %>%
  ...
  kable(
    caption = "Table 2: Fall-Related Injury Costs by Work Environment",
...
comparison_stats %>%
  ...
  kable(
    caption = "Table 3: Injury Cost Distribution by Work Environment",
...
An analysis of workplace injury data reveals that office environments contribute for (`r office_total`%) of total workplace injury costs, with same-level falls (`r same_level_fall`%) being the leading cause of injuries in office settings. In contrast, non-office environments represent a larger share of injury costs at (`r non_office_total`%), primarily due to falls to lower levels (`r lower_level_fall`%) and being struck by objects (`r struck_object`%).

To determine whether these differences in injury cost proportions are statistically significant, I conducted a two-sample t-test. The results showed no significant difference between office and non-office environments, `r t_result`, `r p_result`. lthough office environments had a higher average injury cost proportion (`r office_mean_sd`) compared to non-office environments (`r nonoffice_mean_sd`), the 95% confidence interval `r ci_result` includes zero, suggesting that the observed difference could be due to random chance and is not statistically significant.

```{r office_comparison_plot}
#| echo: false
#| warning: false
#| fig.cap: "Figure 1: Causes of Workplace Injuries"
...
Figure 1 illustrates that office-related falls contribute more than twice the costs of non-office falls (`r office_env`% vs. `r non_office_env`%). This is consistent with the data presented in Table 1, which breaks down injury costs by cause and environment:
...
Table 2 further illustrates that office environments contribute a larger proportion (`r office_prop`%) of fall-related injury costs, despite non-office environments accounting for a greater share of total injury costs.
...
regional_summary %>%
  kable(
    caption = "Table 4: Workplace Injury Rates by Region",
...
high_low_states %>%
  ...
  kable(
    caption = "Table 5: States with the Highest and Lowest Workplace Injury Rates",
...
While injury rates vary across regions, a one-way ANOVA revealed no statistically significant differences in injury rates (`r anova_result`), suggesting that regional variations in injury rates are not substantial enough to impact injury rates in a statistically meaningful way.

I also conducted a Pearson correlation between total injury case volume and injury rates by state. The analysis found no significant correlation (`r cor_result`), indicating that states with higher total injury case volumes do not necessarily experience higher injury rates per 100 workers.

```{r state_comparison_plot}
#| echo: false
#| warning: false
#| fig.cap: "Figure 2: Comparison of Workplace Injury Rates Across Different States"
